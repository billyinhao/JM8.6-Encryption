...
int GetAnnexbNALU (NALU_t *nalu)//处理字节流格式的码流
{
  ...
  while (!StartCodeFound)
  {
    if (feof (bits))
    {
      ...
      nalu->forbidden_bit = (nalu->buf[0]>>7) & 1;         //解码器在处理 7.3.1 小节中的第一个语法元素forbidden_zero_bit    
      nalu->nal_reference_idc = (nalu->buf[0]>>5) & 3;		//优先级
      nalu->nal_unit_type = (nalu->buf[0]) & 0x1f;			//指明当前NAL unit的类型,取低五位
