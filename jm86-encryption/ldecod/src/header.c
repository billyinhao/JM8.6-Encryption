 ...
 */
//++ 参见标准7.3.3
/*进入 RestOfSliceHeader 函数之后，我们可以发现有 ref_pic_list_reordering 函数，
所以可以证明我们的解码器的确是按照标准规定来做的.RestOfSliceHeader 函数后面的
代码所做的工作也都是按照标准的 7.3.3 节做的 */
 ...
 */
//++ 参见标准7.3.3.1
/*7.3.3 规定了解码完片头的语法元素，要进行 ref_pic_list_reordering 操作,
是按照标准的 7.3.3.1 小节规定在做,后面看这个代码都只是一个查找的过程（因为找到标准各部分对应的代码），
到这里大家应该已经将标准第 7 章与代码联系起来了,
第 7 章正是解码器伪代码 */
...
//deeod甘oe()函数计算图像序列号POC(picture order count),根据pic--order--cnt--type值的不同，
//会采用不同的算法来计算Poe
