...
// Notation for comments（注释符号） regarding prediction and predictors.
...
///////////////////////////////////////////////////////////////////////////
static void show_info_pix(PixelPos  pix,char pix_position,byte**imgY)
{
  int i;
  printf("pix %c_availsble: %d\n",pix_position,pix.available);
  printf("pix %c_address: %d\n",pix_position,pix.mb_addr);
  printf("pix %c x: %d y: %d\n",pix_position,pix.x,pix.y);
  printf("pix %c pos_x: %d pos_y: %d\n",pix_position,pix.pos_x,pix.pos_y);
  switch(pix_position)
  {
   case 'b':
   for(i=0;i<BLOCK_SIZE;i++)
     printf("%d ",imgY[pix.pos_y][pix.pos_x+i]);
     putchar('\n');
   break;
   case 'd':
   printf("%d ",imgY[pix.pos_y][pix.pos_x]);
        putchar('\n');
   break;
   case 'a':
  for(i=0;i<BLOCK_SIZE;i++)
  {
   printf("%d ",imgY[pix.pos_y+i][pix.pos_x+i]);
    putchar('\n');
  }
      break;
  }
}
static void show_info_block( PixelPos * pix,int img_block_x,int img_block_y,int ioff,int joff,short block_available_sign,byte **imgY)
{
  printf("cur_block_position:x %d,y %d\n",img_block_x,img_block_y);
  printf("cur_block_position_in_macroblock:x %d,y %d\n",ioff,joff);
  switch(block_available_sign)
  {
  case 6:
  show_info_pix(pix[1],'b',imgY);
  break;
  case 5:
  show_info_pix(pix[0],'a',imgY);
  break;
  case 4:
  show_info_pix(pix[1],'b',imgY);
  show_info_pix(pix[0],'a',imgY);
...
//++ * 11 12 15 16 *
//++ ***************
//解码4x4块时，首先判断是否为帧内编码，如果是，则通过函数intrapred对4x4块帧内解码。
int intrapred(
...
//static int count=0;
PixelPos  pix[3];
int ioff_block,joff_block;
short block_available_sign=0;
//////////////////////////////////////////////////////////////////////////
...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
     show_info_block(pix,img_block_x,img_block_y,ioff_block,joff_block,6,imgY);
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////
   }
    for(j=0;j<BLOCK_SIZE;j++)
      for(i=0;i<BLOCK_SIZE;i++)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      show_info_block(pix,img_block_x,img_block_y,ioff_block,joff_block,5,imgY);
////////////////////////////////////////////////////////////////////////////////////////////////////////////////      
    }
    for(j=0;j<BLOCK_SIZE;j++)
      for(i=0;i<BLOCK_SIZE;i++)
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
      block_available_sign|=block_available_up;
      block_available_sign|=block_available_left<<1;
      block_available_sign|=block_available_up_left<<2;
      show_info_block(pix,img_block_x,img_block_y,ioff_block,joff_block,block_available_sign,imgY);
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    }
...
//++ * 11 12 15 16 *
//++ ***************
//******itrans()对当前块的残差进行反变换，然后再将反变换值和预测值相加得到重构值。接着，将该重构值赋给***********
//******dec_picture->imagY保存。其中de_picture是一个用来存放解码值的解构体，其成员imgy存放亮度解码值。***********
