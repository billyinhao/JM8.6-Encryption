...
int InterpretSPS (DataPartition *p, seq_parameter_set_rbsp_t *sps) 
{
  ...
  //是在从码流中解码 profile_idc,u_v 是一个函数，该函数的作用就是按 7.3.2.1 小节第二行最后一列的 u(8) 规定的编码
  //方式解码 profile_idc,函数u_v中的8就是7.3.2.1 小节第二行最后一列的 u(8) 指定的那个 8

  //下面三行分别在解码 constrained_set0_flag、constrained_set1_flag、constrained_set2_flag，解码器为什么要在
  //这里解码这三个语法元素呢？因为标准的 7.3.2.1 小节规定了 profile_idc 之后是这三个语法元素，所以解码器必须
  //在这里解码这三个语法元素, 那么解码器到底该以什么方式解码这三个语法元素呢？标准的 7.3.2.1 小节第三、四、
  //五行最后一列都规定好了,都采用 u(1),u(1) 在解码器代码中就是 u_1函数.  
  ...
  //InterpretSPS 函数接下来的一长串的代码在做什么？就是在按照 7.3.2.1 小节规定的顺序挨着解码每个语法元素 
                              ...
                              //ProcessPPS 函数里最重要的函数仍然是 InterpretPPS 
{
  ...
  //下面的代码在完全按照标准 7.3.2.2 小节规定的内容解码 PPS 
...
void ProcessSPS (NALU_t *nalu)
{
  ...
  dummy = InterpretSPS (dp, sps);//该函数中最重要的函数是 InterpretSPS 
...
void ProcessPPS (NALU_t *nalu)   //ProcessPPS 函数里最重要的函数仍然是 InterpretPPS 
{
  DataPartition *dp = AllocPartition(1);
