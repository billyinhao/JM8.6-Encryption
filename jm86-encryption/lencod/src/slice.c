...
int start_slice()
{
  ...
  init_ref_pic_list_reordering();//初始化参考帧列表重排序标识
...
int encode_one_slice (int SliceGroupId, Picture *pic)
{
  ...
  set_ref_pic_num();//设置参考帧序号

  if (img->type == B_SLICE)
      compute_collocated(Co_located, listX);

  CurrentMbAddr = FmoGetFirstMacroblockInSlice (SliceGroupId);//取得当前编码条带/(条带组)的第一个宏块地址
...
static Slice *malloc_slice()
{
  ...
  assignSE2partition[0] = assignSE2partition_NoDP;//语法元素对应的数据分割
	//ZL 
