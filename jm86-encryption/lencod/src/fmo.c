...
static int FirstMBInSlice[MAXSLICEGROUPIDS];//条带组中当前编码条带的第一个宏块地址??

int *MBAmap = NULL;   //(光栅扫描的宏块序号到条带组的映射关系)
...
int FmoInit(ImageParameters * img, pic_parameter_set_rbsp_t * pps, seq_parameter_set_rbsp_t * sps)
{
  ...
  FmoGenerateMapUnitToSliceGroupMap(img, pps);//确定片到片组的映射关系
  FmoGenerateMBAmap(img, sps);//确定宏块到片的映射
