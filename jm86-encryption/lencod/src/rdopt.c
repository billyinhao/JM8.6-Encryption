...
int Mode_Decision_for_4x4IntraBlocks (int  b8,  int  b4,  double  lambda,  int*  min_cost)
{
  ...
  double  rdcost;
  int     block_x     = 8*(b8%2)+4*(b4%2);//0,4,8,12,
  int     block_y     = 8*(b8/2)+4*(b4/2);
  int     pic_pix_x   = img->pix_x+block_x;
  int     pic_pix_y   = img->pix_y+block_y;
                             ...
                             int     bwd_ref)    // <-- abp type
{
  ...
  if (!direct)
  {
    ...
    if (pdir==0 || pdir==2)
    {
      rate  += writeMotionVector8x8 (i0, j0, i0+2, j0+2, ref, LIST_0, mode,1);
    }
    if (pdir==1 || pdir==2)
    {
      rate  += writeMotionVector8x8 (i0, j0, i0+2, j0+2, bwd_ref, LIST_1, mode,1);
    }
  }
 ...
 */
 void encode_one_macroblock ()
 {
   ...
   int         curr_cbp_blk, cnt_nonz = 0, best_cnt_nonz = 0, best_fw_ref = 0, best_pdir;//best_fw_ref 最佳前向预测参考帧号
   int         cost=0;
   ...
   int     **ipredmodes = img->ipredmode;//以子宏块长为坐标储存一副图像所有子宏块的帧内预测模式
   int     best_bw_ref = -1;
   ...
   // find out the correct list offsets   //用来便利到ListXSize[2-3/4-5]数组的
   if (curr_mb_field)
   {
   ...
   valid[12]     = (siframe);
//////////////////////////////////////////////////////////////////////////////////////////主要看如果当前编码图像是场编码，或场宏块但是与参考图像与其顶底场
//属性不一致的话，就给chroma_vector_adjustment赋值，这个值好像是为了根据亮度MV计算色度MV的
   if (!img->MbaffFrameFlag)
   {
   ...
   //===== SET LAGRANGE PARAMETERS =====//根据是否启用RDOPT来生成拉格朗日系数lambda_motion_factor
   if (input->rdopt)
   {
   ...
   for (rerun=0; rerun<runs; rerun++)
   {
     ...
     if (!intra)
     {
       ...
       for (min_cost=1<<20, best_mode=1, mode=1; mode<4; mode++)//外层模式循环
       {
         if (valid[mode])//内层分块循环
         {
           for (cost=0, block=0; block<(mode==1?1:2); block++)
           {
             PartitionMotionSearch (mode, block, lambda_motion);//lambda_motion用来求解运动矢量消耗的码率 *********

             //--- set 4x4 block indizes (for getting MV) --- //对于每种分割的后一个分块(block==1)设置相应的4*4块索引??
             j = (block==1 && mode==2 ? 2 : 0);
             i = (block==1 && mode==3 ? 2 : 0);

             //--- get cost and reference frame for forward prediction ---
             for (fw_mcost=max_mcost, ref=0; ref<listXsize[LIST_0+list_offset]; ref++)//fw_mcost:前向预测最佳参考帧的代价
             {
               if (!checkref || ref==0 || CheckReliabilityOfRef (block, LIST_0, ref, mode))//要么不需要检查，要么是前向参考帧列表第一帧，要么检查通过
               {
                 mcost  = (input->rdopt ? REF_COST (lambda_motion_factor, ref, LIST_0 + list_offset) : (int)(2*lambda_motion*min(ref,1)));

                 mcost += motion_cost[mode][LIST_0][ref][block];//motion_cost在PartitionMotionSearch中改变，储存的是运动搜索的代价
                 if (mcost < fw_mcost)
                 {
                   fw_mcost    = mcost;
                   best_fw_ref = ref;
             ...
             if (bframe)
             {
               ...
               {
                 best_pdir = 0;//best_pdir 最佳预测方向0，1，2代表前后双
                 best_bw_ref = 0;
                 cost += fw_mcost;
               }
             ...
             //在某种分块模式下某个分块的最佳预测模式以及参考帧在相应参考帧列表里的编号
/////////////  记录一下在这种模式下当前大分块block对应的4*4小分块的参考帧号参考帧信息和MV信息
//下面这些全局变量用做BlockMotionSearch()中运动搜索时运动向量预测的依据 
             if (mode==1)//16*16
             {
               if (best_pdir==1)
               {
             ...
             //----- set reference frame and direction parameters -----记录每个8*8大分块在相应的分割模式下的预测方向，前后向的最佳参考帧号
             if (mode==3)
             {
             ...
             //--- set reference frames and motion vectors ---//储存模式2、3的第一个分块的信息 用于下一分块的运动矢量预测 与上面重复了
             if (mode>1 && block==0) 
      ...
      if (valid[P8x8])
      {
        ...
        for (cbp8x8=cbp_blk8x8=cnt_nonz_8x8=0, block=0; block<4; block++)//对宏块的4个大分块进行循环
        {
          //--- set coordinates ---
          j0 = ((block/2)<<3);    j1 = (j0>>2);//像是坐标一样东西 
          i0 = ((block%2)<<3);    i1 = (i0>>2);
          
          //=====  LOOP OVER POSSIBLE CODING MODES FOR 8x8 SUB-PARTITION  =====
          for (min_cost8x8=(1<<20), min_rdcost=1e30, index=(bframe?0:1); index<5; index++)//min_cost8x8非RDO模式下的最小代价 min_rdcost是RDO模式下的最小代价
          {
            if (valid[mode=b8_mode_table[index]])
            {
              curr_cbp_blk = 0;//子宏块CBP标识
              
              if (mode==0)//如果是B帧 先进行一下直接预测
              {
                //--- Direct Mode ---
                if (!input->rdopt)
                {
