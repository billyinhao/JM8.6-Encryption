...
void code_a_picture(Picture *pic)
{
  ...
  FmoInit(img, active_pps, active_sps);//确定宏块到片组映射关系的函数
  ...
  while (NumberOfCodedMBs < img->total_number_mb)       // loop over slices
  {
    ...
    while (!FmoSliceGroupCompletelyCoded (SliceGroup))  //FmoGetFirstMacroblockInSlice函数，通过FirstMBInSlice[SliceGroup]数组返回条带组中需要编码的条带的第一个宏块地址，如果为-1.证明该条带组已经编码完成
    {
      // Encode the current slice
      NumberOfCodedMBs += encode_one_slice (SliceGroup, pic);
      FmoSetLastMacroblockInSlice (img->current_mb_nr);//当前条带组所要编码的下一条带的第一个宏块地址写入FirstMbInSlice[]
      // Proceed to next slice
      img->current_slice_nr++;
      stat->bit_slice = 0;                     //把当前编码条带比特数据统计数清零为下一条带编码做准备
    }
...
*/
  ...
  init_frame ();
  FrameNumberInFile = CalculateFrameNumber();  //返回源文件中需要读取的帧的次序
  ...
  else
  {
    ...
    // For field coding, turn MB level field/frame coding flag off ?????????????????错了吧
    if (input->MbInterlace)
      mb_adaptive = 0;
...
static void init_frame ()
{
  ...
  if (img->type != B_SLICE)
  {
    img->tr = start_tr_in_this_IGOP + IMG_NUMBER * (input->jumpd + 1);//处理跳帧情况下的视频帧数 start_tr_in_this_IGOP ==0
    ...
    img->mb_y_intra = img->mb_y_upd;  //  img->mb_y_intra indicates which GOB to intra code for this frame//错误隐藏
