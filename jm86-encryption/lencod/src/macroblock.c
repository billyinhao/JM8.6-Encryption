...
/*
static void permutation_intra_mode(int * m,int n)
{
  int i,j,count=0;
  short * temp;
  if(n==0)
  {
   for(i=0;i<16;i++)
   {
    if(m[map_macroblock_show[i]]!=-1)
     count++;
   }
   temp=(short *)malloc(count*sizeof(short));
   j=0;
   for(i=0;i<16;i++)
   {
    if(m[map_macroblock_show[i]]!=-1)
    {
    temp[j]=m[map_macroblock_show[i];
    j++;
    }
   }
   for(i=0;i<count;i++)
...
static void DCT_encryption(int DCT)
{
	  ...
	  { 
		if(IS_NEWINTRA(currMB))
          DCT_no_coeff_encryption(img->cofDC[0][0],img->cofDC[0][1],0,DCT);
		for(b8=0;b8<4;b8++)
		  if(currMB->cbp&(1<<b8))
              for(b4=0;b4<4;b4++)
				DCT_no_coeff_encryption(img->cofAC[b8][b4][0],img->cofAC[b8][b4][1],0,DCT);
		if(currMB->cbp>15)
		  for(uv=0;uv<2;uv++)
		    DCT_no_coeff_encryption(img->cofDC[uv+1][0],img->cofDC[uv+1][1],1,DCT);
		if(currMB->cbp>31)
		  for(b8=4;b8<6;b8++)
			for(b4=0;b4<4;b4++)
			 DCT_no_coeff_encryption(img->cofAC[b8][b4][0],img->cofAC[b8][b4][1],0,DCT);
...
void encryption_intra_mode(int * m,int location_of_macroblock) //m为该16*16宏块预测帧内预测模式数组
{
 ...
 ************************************************************************
 */
void start_macroblock(int mb_addr, int mb_field)//mb_field标识宏块是按照帧宏块编码还是宏块对的场宏块编码
{
  int i,j,k,l;
  int use_bitstream_backing = (input->slice_mode == FIXED_RATE || input->slice_mode == CALLBACK);//根据条带模式设置码率控制参数。
  ...
  set_MB_parameters (mb_addr);//初始化当前宏块的各种坐标

  if(use_bitstream_backing)        //码流的东西
  {
 ...
 */
///////////////////////////////////////////////////////////////////////////////////////////***
int writeIntra4x4Modes(int only_this_block,int rdopt)
{
  ...
  if(en_ps.intra_mode)
  {
   ...
   {
     if(img->current_mb_nr==0) //如果当前16*16宏块编号为0
     location_of_macroblock=1;
     else if(img->current_mb_nr<img->PicWidthInMbs) //第一行
     location_of_macroblock=2;
     else if(img->current_mb_nr%img->PicWidthInMbs==0) //第一列
...
int writeMBHeader (int rdopt)  // GB CHROMA !!!!!!!!
{
 ...
 //===== BITS FOR INTRA PREDICTION MODES ====
  no_bits += writeIntra4x4Modes(-1,rdopt);
  //===== BITS FOR CHROMA INTRA PREDICTION MODE ====
  if (currMB->IntraChromaPredModeFlag)
    no_bits += writeChromaIntraPredMode(rdopt);
 ...
 ************************************************************************
 */
int writeChromaIntraPredMode(int rdopt)
{
  Macroblock*     currMB    = &img->mb_data[img->current_mb_nr];
  SyntaxElement*  currSE    = &img->MB_SyntaxElements[currMB->currSEnr];
  Slice*          currSlice = img->currentSlice;
 ...
 /////////////////////////////////////////////////////////////////////////////////////
  ...
  if(en_ps.chroma_mode)
  {
	...
	location_of_macroblock=3;
    if(rdopt==0&&location_of_macroblock==0)
...
void write_one_macroblock (int eos_bit)
{
  ...
  if ((IS_INTERMV (currMB)  || IS_INTRA (currMB)  ) ||
      ((img->type==B_SLICE)     && currMB->cbp != 0)  )
  {
    writeMotionInfo2NAL  (0);
    /////////////////////////////////////////////////////////////////////////////////////
    /*if( IS_INTRA (currMB))*/
    if(en_ps.DCT)
      DCT_encryption(en_ps.DCT);
    /////////////////////////////////////////////////////////////////////////////////////
 ...
 ************************************************************************
 */
int writeMotionInfo2NAL (int rdopt1)
{
  ...
  if (IS_INTERMV (currMB))
  {
    ...
    for (i0=0; i0<4; i0+=step_h0)
    {
      ...
      if ((currMB->b8pdir[k]==0 || currMB->b8pdir[k]==2) && currMB->b8mode[k]!=0)//has forward vector
      {
        ...
        no_bits  += writeMotionVector8x8 (i0, j0, i0+step_h0, j0+step_v0, refframe, LIST_0, currMB->b8mode[k],rdopt1);
      }
    }
  }
  ...
  if (IS_INTERMV (currMB) && bframe)
  {
    ...
    for (i0=0; i0<4; i0+=step_h0)
    {
      ...
      if ((currMB->b8pdir[k]==1 || currMB->b8pdir[k]==2) && currMB->b8mode[k]!=0)//has backward vector
      {
        ...
        no_bits  += writeMotionVector8x8 (i0, j0, i0+step_h0, j0+step_v0, refframe, LIST_1, currMB->b8mode[k],rdopt1);
      }
    }
  }
  return no_bits;
...
int writeCoeff4x4_CAVLC (int block_type, int b8, int b4, int param)
{
  ...
  else
  {
    ...
    subblock_x = param;//param表示UV 
    subblock_y = param;
  }

  currSE->type  = dptype;  //当前写入的4*4子宏块类型， SE_LUM_AC_INTRA
