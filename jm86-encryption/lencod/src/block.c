 ...
 ************************************************************************
 */
void intrapred_luma(int img_x,int img_y, int *left_available, int *up_available, int *all_available)//44宏块输入像素在图像中的坐标，返回当前子宏块
{                      //img_x和img_y可能是4*4块左上角像素在图像内的坐标                                                                             //左部上部左上角像素是否可用标识，并进行帧内预测
  int i,j;                                                                                          //对于不同的预测模式和边界可用情况，将第i种预测模式的结果存在mprri00-mprri33
  int s0;
  int PredPel[13];  // array of predictor pels                                 //储存子宏块相邻像素值                       
  byte **imgY = enc_picture->imgY;  // For MB level frame/field coding tools -- set default to imgY //重构图像像素Y值，以横纵坐标记

  int ioff = (img_x & 15);                        //像素的宏块内坐标
  int joff = (img_y & 15);
  int mb_nr=img->current_mb_nr;

  PixelPos pix_a[4];                              //子宏块左边像素，竖排4个，用4个可能是因为在场模式下可能分属不同的块
  PixelPos pix_b, pix_c, pix_d;                   //子宏块上，右上，左上三个像素

  int block_available_up;                         //当前子宏块上部左部左上右上的块是否可用
  int block_available_left;
  int block_available_up_left;
  int block_available_up_right;

  for (i=0;i<4;i++)                                             //输出相邻像素点性质
  {
  ...
  pix_c.available = pix_c.available && !(((ioff==4)||(ioff==12)) && ((joff==4)||(joff==12)));//由于宏块内44子块的预测顺序，在4,8,12,16处的子宏块右上角像素点不可用

  if (input->UseConstrainedIntraPred)                                                       //当标识为真时，帧间预测的宏块不能用于帧内预测
  {
  ...
  *all_available  = block_available_up && block_available_left && block_available_up_left;
  ...
  // form predictor pels
  if (block_available_up)                                  //如果上部子宏块可用，记录横排像素值
  {
  ...
  if (block_available_up_right)                               //如果右上部子宏块可用，记录横排像素值
  {
  ...
  if (block_available_left)                                      //如果左部子宏块可用，记录竖排像素值
  {
  ...
  if (block_available_up_left)                                     //如果左上部子宏块可用，记录单个像素值
  {
  ...
  for(i=0;i<9;i++)
    img->mprr[i][0][0]=-1;                    //将00位置像素的9种预测模式初始化为-1      
 ...
 ************************************************************************
 */
void intrapred_luma_16x16()                                //宏块16*16帧内预测模式，4种结果存在mprr_2_[i]16*16中
{
  int s0=0,s1,s2;
  int s[16][2];                                            //s[i][0]代表垂直预测值，s[i][1]水平预测值
  int i,j;
  ...
  for (j=0;j< MB_BLOCK_SIZE;j++)
  {
    for (i=0;i< MB_BLOCK_SIZE;i++)
    {
      img->mprr_2[PLANE_16][j][i]=max(0,min(255,(iaa+(i-7)*ib +(j-7)*ic + 16)/32));// store plane prediction PLANE_16要限幅
    }
  }
}
 ...
 */
 /*
    函数输入当前宏块的帧内预测模式，将当前宏块的DC系数哈达玛变换以后的量化值，游程零存在img->cofDC[0][0/1]中([yuv][level/run][scan_pos])
                                    将当前宏块的AC系数量化后的值，游程零存在img->cofAC [b8][b4][0]中([8x8block][4x4block][level/run][scan_pos])
                                    反变换反量化以后与预测块相加以后的重构的宏块亮度值，储存在 enc_picture->imgY[img->pix_y+j][img->pix_x+i]中
                                    返回ac_coef指示当前宏块残差变换后是否含有交流分量(cbp????)
 */
int dct_luma_16x16(int new_intra_mode)                           
{
  ...
  int ii,jj;                               //宏块内子宏块索引
  int i1,j1;                               //寻访用变量
  int M1[16][16];                          //储存一个宏块的帧内预测残差值
  int M4[4][4];                            //提取的宏块4*4DC系数
  int M5[4],M6[4];
  int M0[4][4][4][4];                      //按4*4子宏块形式储存残差值
  ...
  int   b8, b4;
  int*  DCLevel = img->cofDC[0][0];                              //[yuv][level/run][scan_pos]变换量化重排的结果储存到img里
  int*  DCRun   = img->cofDC[0][1];
  int*  ACLevel;
  int*  ACRun;
                                            //算各种量化参数
  qp_per    = (currMB->qp-MIN_QP)/6;        //量化参数除6(向下取整)
  qp_rem    = (currMB->qp-MIN_QP)%6;        //量化参数对6取余

  q_bits    = Q_BITS+qp_per;                //qbits=15+QP/6
  
  qp_const  = (1<<q_bits)/3;                //量化偏移f(帧内)
                                        //储存残差值
  for (j=0;j<16;j++)                   
  {
    for (i=0;i<16;i++)
    {
      ...
      M0[i%4][i/4][j%4][j/4]=M1[i][j];                                                     //i%4子宏块内横坐标 i/4宏块内子宏块横坐标
    }
  }
                                  //M0储存每个4*4DCT变换后的值
  for (jj=0;jj<4;jj++)
  {
    for (ii=0;ii<4;ii++)
    {
      for (j=0;j<4;j++)
      {
        ...
        {
          i1=3-i;
          M5[i]=  M0[i][ii][j][jj]+M0[i1][ii][j][jj];         //M5[0]第j列第一个和第四个相加 M5[3]第j列第一个和第四个相减 
          M5[i1]= M0[i][ii][j][jj]-M0[i1][ii][j][jj];         //M5[1]第j列第二个和第三个相加 M5[2]第j列第二个和第三个相减
        }
  ...
  {
    if (img->field_picture || ( mb_adaptive && img->field_mode ))        //当前编码模式是场模式 或者是宏块级帧场自适应的场模式??
    {  // Alternate scan for field coding                                //变换扫描位置
    ...
    level= (abs(M4[i][j]) * quant_coef[qp_rem][0][0] + 2*qp_const)>>(q_bits+1); //量化后的值

    if (level != 0)                                                    //将量化后的值以游程的形式存到*img->cofDC[0][0]中
    {
    ...
    M4[i][j]=sign(level,M4[i][j]);       //存变换量化后的值(带符号)
  }
  DCLevel[scan_pos]=0;                   //16个位置扫完后下一个位置清零???

  // invers DC transform                 //DC系数反变换
  for (j=0;j<4;j++)
  {
    for (i=0;i<4;i++)
      M5[i]=M4[i][j];
  ...
  for (i=0;i<4;i++)
  {
    ...
    for (j=0;j<2;j++)                                                             //反量化后存到每个子宏块的DC位置上去
    {
      j1=3-j;
  ...
  for (jj=0;jj<4;jj++)
  {
    for (ii=0;ii<4;ii++)
    {
      run      = -1;
      scan_pos =  0;
      b8       = 2*(jj/2) + (ii/2);                   //每个宏块的8*8块中的4*4块索引
      b4       = 2*(jj%2) + (ii%2);
      ACLevel  = img->cofAC [b8][b4][0];           
      ACRun    = img->cofAC [b8][b4][1];

      for (coeff_ctr=1;coeff_ctr<16;coeff_ctr++) // set in AC coeff    扫子宏块AC值，量化
      {
 ...
 */
 /*
    函数处理的是貌似是4*4块不用哈达玛变换的那一组
    函数输入子宏块在当前处理宏块中的坐标，返回当前子宏块DCT变换后是否有非零系数的标识，以及coeff_cost非零系数个数量化后的
    幅值大于1的话赋给很大的值等于1的话给很大的值(好像是为了后续判定是不会被弃(不懂))
    而且好像DC系数也被存到img->cofAC[b8][b4][0]里了
*/
int dct_luma(int block_x,int block_y,int *coeff_cost, int old_intra_mode) 
{
  ...
  int qp_const,level,scan_pos,run;                               //ilev
  int nonzero;                                                  //返回看宏块是否有非零系数
  int qp_per,qp_rem,q_bits;

  int   pos_x   = block_x/BLOCK_SIZE;                           //子宏块在宏块中的坐标(以子宏块长为单位)
  int   pos_y   = block_y/BLOCK_SIZE;
  int   b8      = 2*(pos_y/2) + (pos_x/2);                   
  int   b4      = 2*(pos_y%2) + (pos_x%2);
  int*  ACLevel = img->cofAC[b8][b4][0];                        //储存要处理的4*4块的游程幅值
  int*  ACRun   = img->cofAC[b8][b4][1];                        //储存要处理的4*4块的游程零
  ...
  for (coeff_ctr=0;coeff_ctr < 16;coeff_ctr++)
  {
    ...
    if (img->field_picture || ( img->MbaffFrameFlag && currMB->mb_field ))    //扫描顺序
    {  // Alternate scan for field coding
        i=FIELD_SCAN[coeff_ctr][0];
        j=FIELD_SCAN[coeff_ctr][1];
    }
    ...
    level = (abs (img->m7[i][j]) * quant_coef[qp_rem][i][j] + qp_const) >> q_bits; //量化
 ...
 ************************************************************************
 */
int dct_chroma(int uv,int cr_cbp)                                                 //返回更新色度CBP值，0表示4个4*4块的色度系数得零
                                                                                  //1表示至少有一个DC系数，2表示至少有一个4*4块的AC系数不得零
{
  int i,j,i1,j2,ilev,n2,n1,j1,mb_y,coeff_ctr,qp_const,level ,scan_pos,run;        //n1,n2寻访8*8块中大的4*4子块 
  int m1[BLOCK_SIZE],m5[BLOCK_SIZE],m6[BLOCK_SIZE];                               //m1哈达玛
  int coeff_cost;
  int cr_cbp_tmp;
  int nn0,nn1;
  int DCcoded=0 ;
  ...
  int qpChroma=Clip3(0, 51, currMB->qp + active_pps->chroma_qp_index_offset);   //色度量化偏移
  ...
  //     2X2 transform of DC coeffs.  二乘二的哈达玛变换
  ...
  for (coeff_ctr=0; coeff_ctr < 4; coeff_ctr++)
  {
    ...
    if (level  != 0)
    {
      currMB->cbp_blk |= 0xf0000 << (uv << 2) ;    // if one of the 2x2-DC levels is != 0 set the 如果4个DC中有一个不为零，就把相应的4为都设置为
