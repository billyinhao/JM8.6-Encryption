...
#define SHIFT_AC
//#define SHIFT_AC_TEST

#ifdef SHIFT_AC
long shift_ac(long range, long half)
{
	static double x = 0.963;
	double r = 4.0;
	long result = 0;
	static int i = 0;
	x = r * x * (1 - x);
	if (x == 0.5 || x == 0.75)
		x = 0.3698;
	result = (long)((long long)(x * 1e14) % ((half << 1) - range));
	if (result == 0)
		result = 1;
//	printf("%d\n", i++);
	return result;
...
#ifdef SHIFT_AC
	low = shift_ac(range, HALF);
#endif
	
#ifdef SHIFT_AC_TEST
	printf("-----------------------------------------------------------\n");
	printf("original low: %ld\n", low);
	low = shift_ac(range, HALF);
	printf("low: %ld\n", low);
	printf("-----------------------------------------------------------\n");
	system("pause");
