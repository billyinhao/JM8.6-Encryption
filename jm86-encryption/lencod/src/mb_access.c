 ...
 ************************************************************************
 */
int mb_is_available(int mbAddr, int currMbAddr)    //检验宏块是否可用
{
  if ((mbAddr < 0) || (mbAddr > ((int)img->PicSizeInMbs - 1)))            //如果宏块地址超出图像界限不可用
    return 0;

  // the following line checks both: slice number and if the mb has been decoded
  if (!img->DeblockCall)                                                  //如果该图像没有在做滤波
  {
    if (img->mb_data[mbAddr].slice_nr != img->mb_data[currMbAddr].slice_nr)//如果当前宏块的条带号和要检验的宏块条带号不等，也不可用
      return 0;
  }
...
{
  const int mb_nr = img->current_mb_nr;       //当前宏块地址
  Macroblock *currMB = &img->mb_data[mb_nr];  //当前宏块内容

  // mark all neighbors as unavailable
  currMB->mb_available_up   = NULL;           //指向当前宏块上部的宏块设为空
  currMB->mb_available_left = NULL;           //指向当前宏块左部的宏块设为空

  if (img->MbaffFrameFlag)                    //如果当前图像是帧场自适应
  {
    currMB->mbAddrA = 2 * (mb_nr/2 - 1);                                  //返回相邻宏块对top宏块的地址
    ...
    currMB->mbAvailA = mb_is_available(currMB->mbAddrA, mb_nr) && (((mb_nr/2) % img->PicWidthInMbs)!=0);   //判断相邻宏块是否可用，限定左右
    currMB->mbAvailB = mb_is_available(currMB->mbAddrB, mb_nr);                                            //边界  
  ...
  if (currMB->mbAvailA) currMB->mb_available_left = &(img->mb_data[currMB->mbAddrA]);  //如果左部和上部的宏块可用，重新赋给当前块相邻地址
 ...
 ************************************************************************
 */
void get_mb_block_pos (int mb_addr, int *x, int*y)  //得到地址为mb_addr宏块的坐标
{
 ...
 ************************************************************************
 */
void get_mb_pos (int mb_addr, int *x, int*y)//得到地址为mb_addr宏块的像素为单位的坐标
{
 ...
 ************************************************************************
 */
void getNonAffNeighbour(unsigned int curr_mb_nr, int xN, int yN, int luma, PixelPos *pix)//输入当前块地址，相对于此块的原点坐标xN，yN
{                                                                                        //返回在此坐标下的像素信息(所在块地址，是否可用，各种坐标)
  ...
  if ((xN<0)&&((yN>=0)&&(yN<maxWH)))                       //左块A
  {
    pix->mb_addr  = currMb->mbAddrA;
    pix->available = currMb->mbAvailA;
  }
  else
  if (((xN>=0)&&(xN<maxWH))&&(yN<0))                       //上块B
  {
    pix->mb_addr  = currMb->mbAddrB;
    pix->available = currMb->mbAvailB;
  }
  else
  if (((xN>=0)&&(xN<maxWH))&&((yN>=0)&&(yN<maxWH)))         //当前块
  {
    pix->mb_addr  = curr_mb_nr;
    pix->available = 1;
  }
  else
  if ((xN>=maxWH)&&(yN<0))                                  //右上块C
  {
  ...
  if (pix->available || img->DeblockCall)                    //在像素所在块可用或图像正在做滤波的条件下，返回像素的绝对坐标和在宏块内的坐标
  {
 ...
 ************************************************************************
 */
void getAffNeighbour(unsigned int curr_mb_nr, int xN, int yN, int luma, PixelPos *pix) //输入同上述非帧场自适应宏块
{
  Macroblock *currMb = &img->mb_data[curr_mb_nr];
  int maxWH;
  int yM = -1;                                                                        //yM为了计算所属宏块的相对坐标，在不同帧场模式下意义相当于yN
  ...
  if (xN < 0)                                                               //xN<0,yN<0
  {
    if (yN < 0)
    {
      if(!currMb->mb_field)                                                 //currMb=frame
      {
        // frame
        if (curr_mb_nr%2 == 0)
        {
          // top
          pix->mb_addr  = currMb->mbAddrD  + 1;                              //top Macro
          pix->available = currMb->mbAvailD;
           yM      = yN;
        }
        else
    ...
    { // xN < 0 && yN >= 0                                                     
      if ((yN >= 0) && (yN <maxWH))                                             //xN<0&&yN>=0&&yN<maxWH
      {
        if (!currMb->mb_field)
        {
          // frame
  ...
  {
    // xN >= 0
    if ((xN >= 0)&&(xN <maxWH))                           //(xN >= 0)&&(xN <maxWH)&& (yN<0)
    {
      if (yN<0)
      {
        if (!currMb->mb_field)                             //currMb=frame
        {
          ...
          if (curr_mb_nr % 2 == 0)
          {
            //top
            pix->mb_addr  = currMb->mbAddrB;
            // for the deblocker if the current MB is a frame and the one above is a field          //对于滤波器来说，如果当前宏块是帧宏块而上面的是场宏块
            // then the neighbor is the top MB of the pair                                          //相邻宏块为宏块对的顶宏块
            if (currMb->mbAvailB)
            {
      ...
      else
      {
        ...
        if (yN == 0 && img->DeblockCall == 2)                                           //(xN >= 0)&&(xN <maxWH)&& (yN=0)对于滤波边界特殊处理
        {
          pix->mb_addr  = currMb->mbAddrB + 1;
          pix->available = 1;
           yM      = yN - 1;
        ...
        else if ((yN >= 0) && (yN <maxWH))                                               //(xN >= 0)&&(xN <maxWH)&& (yN >= 0) && (yN <maxWH)
        {
          pix->mb_addr  = curr_mb_nr;
          pix->available = 1;
           yM      = yN;
  ...
  if (pix->available || img->DeblockCall)                               //最后得出的相邻坐标值
  {
    pix->x = (xN + maxWH) % maxWH;
 ...
 ************************************************************************
 */
void getNeighbour(int curr_mb_nr, int xN, int yN, int luma, PixelPos *pix)   //输入当前宏块地址，相对坐标。得出像素根据图像是否为帧场自适应
{                                                                            //的一系列坐标
  if (curr_mb_nr<0)
 ...
 ************************************************************************
 */
void getLuma4x4Neighbour (int curr_mb_nr, int block_x, int block_y, int rel_x, int rel_y, PixelPos *pix)    //输入宏块地址，宏块内相对44坐标，临快相对位置
{
