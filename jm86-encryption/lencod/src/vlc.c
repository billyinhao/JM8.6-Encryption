...
int writeSyntaxElement_Level_VLC1(SyntaxElement *se, DataPartition *this_dataPart)
{
  ...
  if (levabs < 8)
  {
    se->len = levabs * 2 + sign - 1;//绝对值8以下查表
    se->inf = 1;
  }
  else if (levabs < 8+8)
  {
 ...
 ************************************************************************
 */
int writeSyntaxElement_Level_VLCN(SyntaxElement *se, int vlc, DataPartition *this_dataPart)//vlc很大可能是suffixLength
{
  int iCodeword;
  int iLength;
  ...
  int shift = vlc-1;
  int escape = (15<<shift)+1;//控制前缀小于15

  int numPrefix = (levabs-1)>>shift;//前缀值(2*levelabs-2)/1<<vlc
