...
typedef enum {
  NO_SLICES, //不设置条带类型
  FIXED_MB,  //设置条带包含固定宏块数
  FIXED_RATE,//设置条带包含字节数
  CALLBACK,
  FMO
} SliceMode;
...
typedef struct pix_pos
{
  int available;
  int mb_addr;
  int x;                                         //像素在所属宏块中的坐标,以像素为单位或子宏块为单位(示不同求解阶段)
  int y;
  int pos_x;                                     //像素在图像中的绝对坐标
  int pos_y;
} PixelPos;
...
{
  int                 currSEnr;                   //!< number of current syntax element 当前语法元素计数
  int                 slice_nr;                    //宏块所属当前图像的条带号
  int                 delta_qp;
  int                 qp ;
  int                 qpsp ;
  int                 bitcounter[MAX_BITCOUNTER_MB];//当前宏块各个语法元素的编码比特计数
  ...
  int                 cbp ;
  int                 cbp_blk ;    //!< 1 bit set for every 4x4 block with coefs (not implemented for INTRA)低16位表示16个4*4块变换量化后系数是否为零
  int                 b8mode[4];   //本宏块每个8*8分块的最佳预测模式
  int                 b8pdir[4];
  ...
  int                 prev_qp;            //前一个宏块的QP
  int                 prev_delta_qp;      //当前宏块与前一个宏块的QP差
  int                 prev_cbp;
  int                 predict_qp;
  int                 predict_error;
...
typedef struct
{
  ...
  int             bits_to_go;         //!< current bitcounter //写入到多少比特 可以进行输出
...
typedef struct
{
  ...
  int                 num_mb;       //!< number of MBs in the slice 已经编码的宏块数
...
byte   **imgY_org;           //!< Reference luma image(以行列横纵坐标寻访)(img->width*img->hight)个一字节空间指针
...
int  mb_adaptive;     //!< For MB level field/frame coding tools 标识是否可以宏块级帧场自适应
...
byte   **imgY_org_top;//原始顶场亮度数据
byte   **imgY_org_bot;

byte  ***imgUV_org_top;
byte  ***imgUV_org_bot;

byte   **imgY_org_frm;           //原始的帧数据
byte  ***imgUV_org_frm;
...
typedef struct
{
  ...
  int RandomIntraMBRefresh;     //!< Number of pseudo-random intra-MBs per picture  每帧强制采用的随机编码数

  int LFSendParameters;        //显式的传播环路滤波的标识
  int LFDisableIdc;
  int LFAlphaC0Offset;
  int LFBetaOffset;
...
typedef struct
{
  int number;                  //!< current image number to be encoded 非B帧编码图像序号
  int pn;                      //!< picture number
  int nb_references;           //
  int current_mb_nr;           //当前编码的宏块号
  int total_number_mb;         //当前编码图像的总宏块数??
  int current_slice_nr;        //一幅图像内当前编码的条带号 计数功能??
  int type;                    //当前处理条带类型
  int structure;               //!< picture structure标识当前正在编码的图像结构是帧还是场
  int num_reference_frames;    //!< number of reference frames to be used参考帧数
  int max_num_references;      //!< maximum number of reference pictures that may occur最大参考帧数 如果是场模式的话2 * active_sps->num_ref_frames;
  ...
  int **ipredmode;             //!< intra prediction mode  一副图像所有宏块的帧内预测模式
  int cod_counter;             //!< Current count of number of skipped macroblocks in a row
  int ***nz_coeff;             //!< number of coefficients per block (CAVLC)每个子宏块非零系数的个数

  int mb_x;                    //!< current MB horizontal                     当前编码宏块坐标
  int mb_y;                    //!< current MB vertical
  int block_x;                 //!< current block horizontal
  int block_y;                 //!< current block vertical
  int pix_x;                   //!< current pixel horizontal                 当前编码像素的坐标
  int pix_y;                   //!< current pixel vertical
  int pix_c_x;                 //!< current pixel chroma horizontal
  int pix_c_y;                 //!< current pixel chroma vertical

  int opix_x;                   //!< current original picture pixel horizontal当前原始图像像素水平坐标(源图像当前宏块的水平坐标，以像素为单位)
  int opix_y;                   //!< current original picture pixel vertical  当前原始图像像素竖直坐标(当前宏块的竖直坐标，以像素为单位)
  ...
  int mprr[9][16][16];         //保存当前宏块每个像素的9种帧内预测的值(貌似只用到了前4*4个)

  int mprr_2[5][16][16];       //!< all 4 new intra prediction modes(保存当前宏块4种帧内预测模式值)
  int mprr_c[2][4][8][8];      //!< new chroma 8x8 intra prediction modes
  int mpr[16][16];             //!< current best prediction mode
  int m7[16][16];              //!< 原始像素与帧内4*4预测后的残差值，色度块残差值8*8
  ...
  Picture     *currentPicture; //!< The coded picture currently in the works (typically frame_pic, top_pic, or bottom_pic)当前编码图像数据
  Slice       *currentSlice;                                //!< pointer to current Slice data struct
  Macroblock    *mb_data;                                   //!<一帧图像所有的宏块数据，以宏块地址寻访(好像不对 是当前编码宏块数据)
  ...
  int *intra_block;         //标识图像中的各个宏块是否为帧内预测块(可用于帧内预测)

  int tr;                     //当前编码的图片在原始视频序列中的序号
  int fld_type;                        //!< top or bottom field
  unsigned int fld_flag;                //最终是采用帧方式还是场方式哪种编码方式的                
  ...
  // B pictures
  int b_interval;
  int p_interval;                 //跳帧时的P帧间隔
  int b_frame_to_code;            //当前编码B帧的序列号
  int fw_mb_mode;
  int bw_mb_mode;

  int****** pred_mv;                 //!< motion vector predictors for all block types and all reference frames

  int****** all_mv;       //!< replaces local all_mv[block_y][block_x][[list][ref][mode][direction]
  int LFDisableIdc;
  int LFAlphaC0Offset;
  int LFBetaOffset;
  ...
  int MbaffFrameFlag;    //!< indicates frame with mb aff coding 标识当前编码帧是否支持宏块级帧场自适应

  //the following should probably go in sequence parameters
  // unsigned int log2_max_frame_num_minus4;
  unsigned int pic_order_cnt_type;   //POC计算类型 0或1//////////////////////////////////////////////////////////////////////////
  // for poc mode 0, POC200301
  // unsigned int log2_max_pic_order_cnt_lsb_minus4;  
  // for poc mode 1, POC200301
  unsigned int delta_pic_order_always_zero_flag;//用来控制delta_pic_order_cnt[0]/[1]在片头出不出现 1不出现0出现
           int offset_for_non_ref_pic;          //用来计算非参考帧POC
           int offset_for_top_to_bottom_field;  //用来计算帧的底场POC
  unsigned int num_ref_frames_in_pic_order_cnt_cycle;//一直是1 (用来解码POC)
           int offset_for_ref_frame[1];  // MAX_LENGTH_POC_CYCLE in decoder                      ///////////各种POC运算参数

  // POC200301
  //the following is for slice header syntax elements of poc
  // for poc mode 0.
  unsigned int pic_order_cnt_lsb;//POC低位值
  ...
  // POC200301
  unsigned int field_picture;           //标识我们当前编码的是一帧还是一场
  ...
  unsigned PicSizeInMbs;//当前正在编码图像(帧或是场)所拥有的宏块数
...
//  int delta_pic_order_cnt[2];
  ...
  /*rate control*/
  int NumberofHeaderBits; //头文件比特数(包括条带头)
...
////////////////////////////////////////////////////////////////////////////////////////////
extern encryption_parameter en_ps;
////////////////////////////////////////////////////////////////////////////////////////////
...
int   writeMotionVector8x8      (int  i0, int  j0, int  i1, int  j1, int  refframe, 
                                 int  list_idx, int  mv_mode,int rdopt);//////////////////////////////////////////////////
int   writeReferenceFrame       (int, int, int, int, int);
int   writeAbpCoeffIndex        (int, int, int, int);
int   writeIntra4x4Modes        (int,int);////////////////////////////////////
int   writeChromaIntraPredMode  (int);///////////////////////////////////
...
int   writeMotionInfo2NAL (int rdpt);//////////////////////////////////////////////////////
