...
typedef struct storable_picture           //感觉是重构的参考图像
{
  ...
  int         frame_poc;
  int         order_num;
  int64       ref_pic_num[6][MAX_LIST_SIZE];//储存的是不同参考帧列表中的不同参考帧的信息(好像等于POC的二倍)
  ...
  byte **     imgY;          //!< Y picture component 以像素在图像中的绝对坐标寻访
  ...
  byte *      mb_field;      //!< field macroblock indicator //只是参考图像中的宏块那个是按照场宏块模式编码的
  ...
  int  ****   mv;            //!< motion vector       [list][subblock_x][subblock_y][component]//当前编码子宏块在某一编码模式下在相应参考帧列表中(最佳参考帧)的运动矢量
